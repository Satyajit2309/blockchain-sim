...
class Blockchain:
    def __init__(self, admins: Optional[List[str]] = None):
        ...
        self.votes: Dict[str, Dict[str,str]] = {}  # tx_id -> {admin: vote}
        self.load_from_file()
    ...
    def new_transaction(self, tx: Dict[str,Any]) -> str:
        """
        Add a new pending transaction. Returns tx_id.
        tx must include: type, payload (e.g. vehicle data), requested_by
        """
    ...
    def cast_vote(self, tx_id: str, admin: str, vote: str) -> Dict[str,Any]:
        """
        admin votes 'approve' or 'reject' on a pending transaction.
        If majority reached -> finalize block (if approved) or mark rejected.
        """
        ...
        if approve_count >= majority:
            ...
            if tx_record:
                ...
                # do not add a block, but mark rejected
                self.pending_transactions = [t for t in self.pending_transactions if t["tx_id"] != tx_id]
                del self.votes[tx_id]
                self.save_to_file()
    ...
    def _add_block(self, transactions: List[Dict[str,Any]]):
        ...
        # Simulated "mining" / finalization step: increment nonce until hash starts with '00' (lightweight)
        target_prefix = "00"
    ...
    def load_from_file(self):
        try:
            ...
            for b in data.get("chain", []):
                ...
                # ensure hash matches stored one (but recompute to be consistent)
